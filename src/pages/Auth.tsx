  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        >
          <div className="rounded-[20px] bg-foreground text-background px-6 py-6 relative shadow-2xl flex flex-col items-center">
            {/* Back arrow */}
              ...
              <motion.div key="main" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                >
                  <Button onClick={() => handleSocialLogin("google")} disabled={loading}
                    className="rounded-[20px] h-11 w-full text-sm font-medium gap-2 bg-background text-foreground hover:bg-background/90">
                  ...
                  <Button onClick={() => handleSocialLogin("apple")} disabled={loading}
                    className="rounded-[20px] h-11 w-full text-sm font-medium gap-2 bg-transparent border border-background/30 text-background hover:bg-background/10">
                ...
                >
                  <form onSubmit={handleSignInSendOtp} className="space-y-3">
                    ...
                    <div className="flex justify-center">
                      <Button type="submit" disabled={loading} className="rounded-[20px] h-11 px-6 text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90">
                  ...
                  <form onSubmit={handleSignUp} className="space-y-2.5">
                    <div className="relative">
                      ...
                      <Input type="text" placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)}
                        className="h-11 rounded-[20px] pl-10 bg-background text-foreground border-0 text-sm" maxLength={100} autoFocus />
                    </div>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/40" />
                      <Input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)}
                        className="h-11 rounded-[20px] pl-10 bg-background text-foreground border-0 text-sm" maxLength={255} />
                    </div>
                    <PhoneInput international defaultCountry={defaultCountry as any} value={phone} onChange={setPhone} className="auth-phone-input" />
                    <div className="relative">
                      ...
                      <Input type="password" placeholder="Password (min 6)" value={password} onChange={(e) => setPassword(e.target.value)}
                        className="h-11 rounded-[20px] pl-10 bg-background text-foreground border-0 text-sm" maxLength={128} />
                    </div>
                    {error && <p className="text-xs text-red-400 text-center">{error}</p>}
                    {successMessage && <p className="text-xs text-primary text-center">{successMessage}</p>}
                ...
                >
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-3">
                    <Input type="text" inputMode="numeric" placeholder="000000" value={otp}
                      ...
                      className="h-11 rounded-[20px] text-center text-lg tracking-[0.3em] bg-background text-foreground border-0 font-mono"
