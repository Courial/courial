...
function buildInfoContent(address: string, placeName?: string | null): string {
  const style = "font-family:'Avenir','Avenir Next','Nunito Sans',system-ui,sans-serif;font-size:10px;font-weight:400;color:rgba(0,0,0,0.75);padding:4px 7px 4px 3px;line-height:1.35;max-width:165px;word-wrap:break-word;white-space:normal;overflow-wrap:break-word;";
  if (placeName) {
      ...
      directionsService.route(
        ...
        (result, status) => {
          if (status === "OK" && result) {
            ...
            const tripStyle = "font-family:'Avenir','Avenir Next','Nunito Sans',system-ui,sans-serif;padding:2px 4px;line-height:1.4;text-align:left;white-space:nowrap;";
            const tripContent = `<div class="trip-info-root" style="${tripStyle}"><div style="font-size:11px;font-weight:600;color:rgba(0,0,0,0.85);margin-bottom:0;">Trip Info</div><div style="font-size:10px;font-weight:400;color:#000;">${mins} mins · ${Math.round(totalDistMeters / 1609.34)} mi</div></div>`;
  ...
  return (
    <>
      <style>{`
        ...
        .gm-style-iw:has(.trip-info-root) { padding: 2px !important; min-height: 0 !important; }
        .gm-style-iw:has(.trip-info-root) .gm-style-iw-d { padding: 0 !important; margin: 0 !important; overflow: hidden !important; }
